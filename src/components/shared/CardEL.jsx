...
function CardEL({title, slug, coverPhoto, author}) {
  return (
    <Card sx={{ boxShadow:"rgba(0,0,0,0.1) 0px 4px 12px", borderRadius: 4 }}>
      {author && (
        <CardHeader avatar={<Avatar sx={{marginLeft:2}} src={author.avatar.url}/>} title={<Typography component="p" variant='p' color="text.promary">{author.name}</Typography>}/>
      )}
      
      <CardMedia component="img" height="194" image={coverPhoto.url} alt={slug}/>
      <CardContent>
        <Typography component="h4" variant='h6' color="text.promary" fontWeight={500}>
          {title}
        </Typography>
      </CardContent>
